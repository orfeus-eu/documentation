...
 
This CRG aims also at gathering multiple methodologies that use surface waves (Rayleigh and/or Love) for 2D and 3D imaging of the crustal and upper mantle structure in the region. 
...
CRG 'Ambient Noise Sources'
---------------------------------------
	- Chair: `Jordi Diaz <jdiaz@geo3bcn.csic.es>`_.
	
Description of topic
^^^^^^^^^^^^^^^^^^^^^^^^^
It is well known that seismic data contain signals arising from multiple natural and anthropogenic processes unrelated to tectonic activity, ranging from Earth tides to meteorological phenomena and human activity. The objective of this research group is to bring together AdriaArray researchers working on the investigation of such noise sources. 

The analysis of seismic signals generated by natural processes has led to the development of Environmental Seismology, a field dedicated to providing new insights into phenomena such as ocean-wave dynamics, floods in mountain rivers, snow avalanches, thawing periods, and rainfall events. This approach has opened the door to interdisciplinary studies in hydrology, biology, and meteorology.

Seismic data can also be used to monitor processes associated with human activity, including vehicle traffic, mining operations, and cultural events such as rock concerts or football matches. Beyond the scientific interest in understanding how these sources generate and propagate seismic waves, such studies offer a powerful tool for communicating Earth sciences to a broader audience.

In addition, identifying stations affected by specific noise sources during episodic time intervals can help prevent misinterpretations in ambient noise tomography, as these methods assume that the sources of ambient vibrations are uniformly distributed. Deviations from this assumption must be taken into account when interpreting results.

Therefore, we are convinced that investigating the sources of ambient noise is of great relevance for a project like AdriaArray, which involves a large number of stations distributed across a continental‑scale network.

In addition, characteristics of the near-surface structure beneath the stations can be obtained from seismic ambient noise. We will systematically analyse HVSR for AdriaArray stations. Inversions for Vs, Vp, and density will be performed.
